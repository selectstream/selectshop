...
const products = defineCollection({
			...
			return data.results.map((page: any) => {
				...
				return {
					...
					mesh: {
						...
						weight: props['Weight (g)']?.number || 0,
						weightLimit: props['Weight Limit (g)']?.number || 0,
						interfaceType: props['Interface Type']?.select?.name || '',
	...
	schema: z.object({
		...
		digitalStack: z.string().optional(),
		stackId: z.string().optional(),
		stackName: z.string().optional(),
		weight: z.number().default(0),
		weightLimit: z.number().default(0),
